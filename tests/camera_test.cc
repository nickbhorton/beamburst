...
int main()
{
    ...
    for (const auto& pixel_jobs : pixel_jobs_vector) {
        for (const auto& [x, y, position, normal, view] : pixel_jobs) {
            ...
            [[maybe_unused]] vec3 gn = to_tangent_space(normalize(grad_norm(
                fract(longitude * 16.0),
                fract(latitude * 8.0),
                0.26,
                0.5,
                0.5,
            ...
            auto const& [diffuse, specular] =
                blin_phong(light_position, position, view, gn, 1.0);

            vec3 ambient_color = {longitude, 0.0, latitude};
